// Copy of asn1 go 1.18.4 fixing parseGeneralizedTime to allow second fractions
//
...
func parseGeneralizedTime(bytes []byte) (ret time.Time, err error) {
	const formatStr = "20060102150405.999Z0700"
	s := string(bytes)
...
func parseSequenceOf(bytes []byte, sliceType reflect.Type, elemType reflect.Type) (ret reflect.Value, err error) {
	...
	for offset := 0; offset < len(bytes); {
		...
		if !matchAny && (t.class != asn1.ClassUniversal || t.isCompound != compoundType || t.tag != expectedTag) {
...
func parseField(v reflect.Value, bytes []byte, initOffset int, params fieldParameters) (offset int, err error) {
	...
	case *asn1.RawValue:
		*v = asn1.RawValue{t.class, t.tag, t.isCompound, innerBytes, bytes[initOffset:offset]}
		return
