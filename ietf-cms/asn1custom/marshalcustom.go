...
func makeBody(value reflect.Value, params fieldParameters) (e encoder, err error) {
	...
	case timeType:
		...
		if params.timeType == asn1.TagGeneralizedTime || outsideUTCRange(t) {
...
func makeField(v reflect.Value, params fieldParameters) (e encoder, err error) {
	...
	if params.timeType != 0 && tag != asn1.TagUTCTime {
		return nil, asn1.StructuralError{"explicit time type given to non-time member"}
	}

	if params.stringType != 0 && tag != asn1.TagPrintableString {
	...
	case asn1.TagUTCTime:
		if params.timeType == asn1.TagGeneralizedTime || outsideUTCRange(v.Interface().(time.Time)) {
	...
	if params.tag != nil {
		...
		if params.explicit {
			t.tag = bytesEncoder(appendTagAndLength(t.scratch[:0], tagAndLength{asn1.ClassUniversal, tag, bodyLen, isCompound}))
